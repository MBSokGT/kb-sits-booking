...
Миграция корпоративного домена почты (опционально):

- `LEGACY_EMAIL_DOMAIN_MAP` — список пар `старый_домен:новый_домен` через
  запятую, например `kbmik.ru:pinchdrop.ru,vsedb.ru:brs-impex.ru`.
  Если сотрудник входит под новым адресом, а аккаунт всё ещё числится под
  старым доменом, сервис находит именно его — роль, история бронирований и
  внутренний id сохраняются, меняется только email. Работает и для обычного
  входа по паролю, и для входа через LDAP.
...
3. `BASE DN` и DNs групп ролей:
   ...
   - бухгалтерия: `KB-Booking-Accounting`
...
LDAP_ROLE_ACCOUNTING_GROUP_DN=CN=KB-Booking-Accounting,OU=Groups,DC=company,DC=local
LDAP_DEFAULT_DEPARTMENT=Домен
LDAP_FALLBACK_EMAIL_DOMAIN=company.local
LDAP_TLS_REJECT_UNAUTHORIZED=1
```

Роль «Бухгалтерия» (`accounting`) видит только раздел статистики бронирований
(с фильтрами по отделам/датам и выгрузкой отчёта для бухгалтерии) — без карты,
своих бронирований и админки.

⚠️ **Известное расхождение с реализацией:** `LDAP_ROLE_*_GROUP_DN` сейчас
влияют только на то, какая роль будет вычислена при входе, но эта вычисленная
роль нигде не применяется к пользователю — ни при первом автосоздании профиля
(там всегда жёстко ставится `user`), ни при последующих входах (сохраняется то,
что уже есть в БД). Роль всегда нужно назначать вручную в
Администрирование → Пользователи, независимо от групп AD, пока это не
починено в коде.
...
3. Роль в сервисе вычисляется по группам AD, но пока не применяется автоматически (см. предупреждение выше) — назначайте её вручную.
